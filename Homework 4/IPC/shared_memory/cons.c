/****************************************************************************************************************************************/
/*				File Name : Inter-Process Communication using Shared Memory, Consumer process				*/
/*				Author : Sarthak Jain											*/
...
int main(int argc, char* argv[])
{
	...
	if((ptr = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0)) == MAP_FAILED)
	{
	...
	for(int i=0; i<10; i++)
	{
		...
		fprintf(file_ptr, "\nConsumer process received: %ld bytes: %s", strlen(string.messages), string.messages);
	}
	fclose(file_ptr);
